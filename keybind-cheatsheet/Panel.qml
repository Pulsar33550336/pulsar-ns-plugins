...
Item {
  ...
  // Symbol display toggles (end-4 Nerd Font maps)
  readonly property bool useMacSymbol: cfg.useMacSymbol ?? defaults.useMacSymbol ?? false
  readonly property bool useFnSymbol: cfg.useFnSymbol ?? defaults.useFnSymbol ?? false
  readonly property bool useMouseSymbol: cfg.useMouseSymbol ?? defaults.useMouseSymbol ?? false
  readonly property string superKeyText: cfg.superKeyText ?? defaults.superKeyText ?? ""
  ...
  Rectangle {
    ...
    Rectangle {
      ...
      RowLayout {
        ...
        NText {
          text: pluginApi?.tr("panel.title-hyprland")
  ...
  Component {
    ...
    RowLayout {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            NText {
              id: keyText
              anchors.centerIn: parent
              text: root.formatKeyName(modelData)
              font.family: Settings.data.ui.fontFixed
              font.pointSize: root.formatKeyName(modelData).length > 6 ? 8 : 9
              font.weight: Font.Bold
      ...
      // Undescribed bind: static placeholder (no editing/hide actions)
      NText {
        visible: itemData.undescribed
        Layout.fillWidth: true
        Layout.alignment: Qt.AlignVCenter
  ...
  function formatKeyName(key) {
    if (key === "Super" && root.superKeyText) return root.superKeyText;

    if (root.useMacSymbol) {
      var symbol = root.macSymbolMap[key];
      if (symbol) return symbol;
    }
    if (root.useFnSymbol) {
      symbol = root.functionSymbolMap[key];
      if (symbol) return symbol;
    }
    if (root.useMouseSymbol) {
      symbol = root.mouseSymbolMap[key];
      if (symbol) return symbol;
    }

    symbol = root.keySubstitutions[key];
    if (symbol) return symbol;
